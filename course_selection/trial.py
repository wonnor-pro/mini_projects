...
def main():
    ...
    # defo_courses = ["3F1", "3F2", "3F3", "3F4", "3F7", "3C5", "3C6", "3M1"] #
    defo_courses = ["3F1", "3F3", "3F7", "3F8", "3G3", "3G4", "3M1"]

    # Courses that you kinda want to take but not a strong desire
    # want_courses = ["3F8", "4M12", "3B2"] #
    want_courses = ["3B6", "3E1", "3E3"]

    # Courses that you find okay to take if necessary
    # okay_courses = ["3B1"] # ["3G1", "3G5"] #
    okay_courses =  ["3G1", "3G5"]


    preferred_areas = [5,7,6,1] #[5,6,8,7,4]

    IB_student = student(defo_courses, want_courses, okay_courses, preferred_areas)
    IB_student.load_course_information("course_info.xls")
    IB_student.load_area_information(total_area, "area_info.xlsx")
    IB_student.optimise()
    ...
    print()
    for i in IB_student.M_candidate_courses:
        print(i)
    for i in IB_student.L_candidate_courses:
        print(i)
    # for i in IB_student.areas_list:
    #     print(i)
